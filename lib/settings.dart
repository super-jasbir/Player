...
class _SettingScreenState extends State<SettingScreen> {
  ...
  /// Location keeps its existing behaviour: switching it on opens the
  /// permission screen. It cannot be revoked from inside the app.
  Future<void> _onLocationChanged(bool value) async {
    setState(() => _locationEnabled = value);
    await SharedPref.saveLocationEnabled(value);
    if (value) Get.to(PermissionTwo());
  }

  Future<void> _onBiometricChanged(bool value) async {
    setState(() => _biometricLock = value);
    await SharedPref.saveBiometricLock(value);
  }
