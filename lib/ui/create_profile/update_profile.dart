...
class UpdateProfileController extends BaseController{
  ...
  TextEditingController nricC =TextEditingController();
  ...
  var agree = false.obs;
  var path = "".obs;
  var appLanguage = "local".obs;
  var selectedNation = "singapore".obs;
  ...
  createProfile(){
    var req = {
      ...
      "originOfCountry":(appLanguage.value == "local") ?  "Singapore" : "outside_singapore",
      "userID": userId.text,
    ...
    apiService.postRequest(ApiEndPoint.updateProfile, req,isBearer: true).then((value) {
      if(value.data !=null){
        var json = value.data;
  ...
  getProfileInfo(){
    appController.getProfile(() async {
      fullNameC.text = appController.profileData?.name ?? "";
      nickNameC.text = appController.profileData?.nickName ?? "";
      emailC.text = appController.profileData?.email ?? "";
      selectedDialCode = appController.profileData?.countryCode ?? "";

      mobileController.text = appController.profileData?.phoneNumber ?? "";
      dobC.text = appController.profileData?.dateOfBirthString ?? "";
      addressC.text = appController.profileData?.address ?? "";
      unitC.text = appController.profileData?.unit_number ?? "";

      if(appController.profileData?.originOfCountry == "Singapore"){
        appLanguage.value = "local";
      }else{
        appLanguage.value = "global";
      }

      selectedNation.value = appController.profileData?.originOfCountry?.toLowerCase() ?? "";
      selectedFoodType.value = appController.profileData?.foodType ?? "";

      passPortC.text = appController.profileData?.passportNumber ?? "";
      userId.text = appController.profileData?.userId ?? "";
      nricC.text = appController.profileData?.nric_number ?? "";
      ageInYear.text =  AppUtils.calculateAge(appController.profileData?.dateOfBirth ?? DateTime.now()).toString();

      uploadedProfileImage = appController.profileData?.profilePic ?? "";
      appController.uploadedImage.value = appController.profileData?.profilePic ?? "";
